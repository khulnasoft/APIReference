...
async function formatPackage(filepath: string) {
  ...
  if (
    !data.name.startsWith('@apireference/') &&
    !data.name.startsWith('@apireference-examples/')
  ) {
    printColor(
      'yellow',
      `Package ${data.name} is not in the @apireference/* or @apireference-examples/* scope.`,
    )
  }
