import type { ReferenceConfiguration } from '@apireference/api-reference'
...
const getJavaScriptUrl = (routePrefix?: string) =>
  `${routePrefix ?? ''}/@apireference/fastify-api-reference/js/browser.js`.replace(
    /\/\//g,
    '/',
  )
...
/**
 * The HTML to load the @apireference/api-reference JavaScript package.
 */
...
> = async (fastify, options) => {
  ...
  if (
    ...
    fastify.log.warn(
      '[@apireference/fastify-api-reference] You didn’t provide a spec.content or spec.url, and @fastify/swagger could not be found. Please provide one of these options.',
    )
