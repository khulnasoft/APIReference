...
const getSpecUrl = () => {
  ...
  if (specUrlElement) {
    console.warn(
      '[@apireference/api-reference] The [data-spec-url] HTML API is deprecated. Use the new <script id="api-reference" data-url="/scalar.json" /> API instead.',
    )
...
const getSpec = (): Record<string, any> | undefined => {
  ...
  if (specElement) {
    console.warn(
      '[@apireference/api-reference] The [data-spec] HTML API is deprecated. Use the new <script id="api-reference" type="application/json">{"openapi":"3.1.0","info":{"title":"Example"},"paths":{}}</script> API instead.',
    )
